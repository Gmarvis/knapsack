...
selectIterms.addEventListener('change', () => {
  ...
  //
  const intoBag = iterms.find((iterm) => iterm.name === selectIterms.value)
  ...
  if (selectedWeight + intoBag.weigth > maxWeight) {
    doneBtn.style.backgroundColor = 'red'
    errorText.innerHTML = `<p>You have exceeded maximum Sack Capasity, pick iterm with a lesser wieght between <span>${0}Kg</span> and <span>${
      max - selectedWeight
    }Kg</span></p>`
  } else {
    ...
    for (const iterm of itermsSeleced) {
      list += `<p>${iterm.name} ${iterm.weigth}Kg ${iterm.value}</p>`
    }
...
doneBtn.addEventListener('click', () => {
  ...
  resultsDiv.innerHTML = `<p>weight ocupied: ${selectedWeight}kg | Carring capacity: ${max}kg</p>`
  let list = ''

  for (const iterm of itermsSeleced) {
    list += `<p>${iterm.name} ${iterm.weigth}Kg ${iterm.value}</p>`
  }
  dpresults.innerHTML = list
})
